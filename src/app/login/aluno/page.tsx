...
export default function LoginAlunoPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.user.tipoUsuario !== 'aluno') {
        throw new Error('Esta área é exclusiva para Alunos');
      }

      // Redirecionar para app do aluno
        ...
        <CardContent className="p-8">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <div className="text-center text-sm text-muted-foreground pt-4">
              Não tem uma conta?{' '}
              <Link href="/cadastro-aluno" className="text-destructive hover:underline font-medium">
                Cadastre-se
              </Link>
            </div>
          </form>
